...
import { ArrowLeft, Lock, Shield, Eye, EyeOff, Loader } from "lucide-react";
...
const SecuritySetup = ({ onNext, onBack, initialData }: SecuritySetupProps) => {
  ...
  const debouncedValidate = useMemo(
    () =>
      debounce((data: SecurityData, touched: boolean) => {
  ...
  const validateFormData = useCallback((data: SecurityData): { isValid: boolean; newErrors: Errors } => {
    const newErrors: Errors = {};

    if (!data.securityQuestion) {
      newErrors.securityQuestion = t("security_question_required");
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('securitySetup_draft', JSON.stringify(formData));
    } catch (error) {
  ...
  const handleSubmit = useCallback(
    async (e?: React.FormEvent) => {
      ...
      try {
        const sanitizedData = {
          ...formData,
          securityQuestion: sanitizeInput(formData.securityQuestion),
          securityAnswer: sanitizeInput(formData.securityAnswer.trim()),
          updatedAt: new Date().toISOString(),
        };

        await Preferences.set({
          key: `user_security_${user.uid}`,
          value: JSON.stringify(sanitizedData),
        });

        localStorage.removeItem('securitySetup_draft');

        toast({
          title: t("security_saved"),
          description: t("security_updated_success"),
        });
        ...
        if (error.code === 'unavailable' || !navigator.onLine) {
          ...
          localStorage.setItem('securitySetup_pending', JSON.stringify(formData));
    ...
    [formData, onNext, toast, t, validateFormData, user]
  );
  ...
  const handleBlur = (field: keyof SecurityData) => {
    setFieldTouched(prev => ({ ...prev, [field]: true }));
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            <div>
              ...
              {errors.securityQuestion && (
                <p id="securityQuestion-error" className="text-red-500 text-xs mt-1" role="alert">
                  {errors.securityQuestion}
                </p>
              )}
            </div>
